...
function notify_begin()
{
  if [[ "$ENABLE_EMAIL_NOTIFY" == "all" || "$ENABLE_EMAIL_NOTIFY" == "start" ]]; then
    printf "Subject: Cmbackup - Backup routine for %s start at %s\nGreetings Administrator,
    ...
    \n\nRegards,
    \nCmbackup Team" "$1" "$(date)" "$2"> "$MESSAGE"
    ERR=$( (sendmail -f "$EMAIL_SENDER" "$EMAIL_NOTIFY" < "$MESSAGE" ) 2>&1)
    BASHERRCODE=$?
    if [[ $BASHERRCODE -eq 0 ]]; then
      logger -i -p local7.info "Cmbackup: Mail sent to $EMAIL_NOTIFY to notify about the backup routine begin."
    else
      logger -i -p local7.info "Cmbackup: Cannot send mail for $EMAIL_NOTIFY - $ERR."
    fi
  fi
}
...
#        - FAILURE - For some reason Cmbackup can't conclude this session;
#        - SUCCESS - Cmbackup concluded the session with no problem;
...
function notify_finish()
{
  if [[ "$ENABLE_EMAIL_NOTIFY" == "all" ]] || [[ "$ENABLE_EMAIL_NOTIFY" == "finish" && "$3" == "SUCCESS" ]] || [[ "$ENABLE_EMAIL_NOTIFY" == "error" && "$3" == "FAILURE" ]] ; then
    ...
    # The message
    printf "Subject: Cmbackup - Backup routine for %s complete at %s - %s\nGreetings Administrator,
    ...
    BASHERRCODE=$?
    if [[ $BASHERRCODE -eq 0 ]]; then
      logger -i -p local7.info "Cmbackup: Mail sent to $EMAIL_NOTIFY to notify about the backup routine conclusion."
    else
      logger -i -p local7.info "Cmbackup: Cannot send mail for $EMAIL_NOTIFY - $ERR."
    fi
  fi
